 ...
 */
public class ErrorsPlot extends PlotBox {
    ///////////////////////////////////////////////////////////////////
    ////                         public methods                    ////
    ...
    public synchronized void addPoint(final int dataset, final double x,
        ...
        Runnable doAddPoint = new Runnable() {
            public void run() {
                _addPoint(dataset, x, y, 0, 0, 0, 0, connected, false, false);
            }
        };
     ...
     *   point.
     */
    public synchronized void addPointWithFluxErrorBars(final int dataset,
            final double x, final double y, final double yLowEB,
            final double yHighEB, final boolean connected) {
        Runnable doAddPoint = new Runnable() {
            public void run() {
                _addPoint(dataset, x, y, yLowEB, yHighEB, 0, 0, connected, false, true);
            }
        };
    ...
    public synchronized void addPointWithWaveErrorBars(final int dataset,
            final double x, final double y, final double xLowEB,
            final double xHighEB, final boolean connected) {
        Runnable doAddPoint = new Runnable() {
            public void run() {
                _addPoint(dataset, x, y, 0, 0, xLowEB, xHighEB, connected, true, false);
            }
        };
    ...
    public synchronized void addPointWithDoubleErrorBars(final int dataset,
            final double x, final double y, final double yLowEB,
            final double yHighEB, final double xLowEB, final double xHighEB,
            final boolean connected) {
        Runnable doAddPoint = new Runnable() {
            public void run() {
                _addPoint(dataset, x, y, yLowEB, yHighEB, xLowEB, xHighEB, connected, true, true);
            }
        };
    ...
    public synchronized void setMarksStyle(String style) {
        ...
        if (style.equalsIgnoreCase("none")) {
            ...
            _marks = 3;
        } else if (style.equalsIgnoreCase("big dots")) {
            _marks = 4;
    ...
    public synchronized void writeData(PrintWriter output) {
        ...
        for (int dataset = 0; dataset < _points.size(); dataset++) {
            ...
            for (int pointnum = 0; pointnum < pts.size(); pointnum++) {
                ErrorsPlotPoint pt = (ErrorsPlotPoint) pts.elementAt(pointnum);
                ...
                if (pt.yErrorBar) {
                    output.print(" lowYErrorBar=\"" + pt.yLowEB
                            + "\" highYErrorBar=\"" + pt.yHighEB + "\"");
                }

                if (pt.xErrorBar) {
                    output.print(" lowXErrorBar=\"" + pt.xLowEB
                            + "\" highXErrorBar=\"" + pt.xHighEB + "\"");
                }
    ...
    public synchronized void writeFormat(PrintWriter output) {
        ...
        case 4:
            defaults.append(" marks=\"big dots\"");
            break;
    ...
    /** Draw an error bar for the specified yLowEB, yHighEB values.
     *  If the specified point is below the y axis or outside the
     *  x range, do nothing.  If the <i>clip</i> argument is true,
     *  then do not draw above the y range.
     *  This method should be called only from the event dispatch thread.
    ...
    protected void _drawYErrorBar(Graphics graphics, int dataset, long xpos, long ypos,
    ...
    /** Draw an error bar for the specified xLowEB, xHighEB values.
     *  If the specified point is below the y axis or outside the
     *  x range, do nothing.  If the <i>clip</i> argument is true,
     *  then do not draw above the y range.
     *  This method should be called only from the event dispatch thread.
     ...
     *  @param dataset The index of the dataset.
     *  @param xpos The x position.
     *  @param xLowEBPos The lower y position of the error bar.
     *  @param xHighEBPos The upper y position of the error bar.
     *  @param clip If true, then do not draw above the range.
     */

    protected void _drawXErrorBar(Graphics graphics, int dataset, long xpos, long ypos,
            long xLowEBPos, long xHighEBPos,
            boolean clip) {
        ...
        _drawLine(graphics, dataset,
                xLowEBPos, ypos - _ERRORBAR_LEG_LENGTH,
                xLowEBPos, ypos + _ERRORBAR_LEG_LENGTH, clip);

        _drawLine(graphics, dataset,
               xLowEBPos, ypos,
                xHighEBPos, ypos, clip);

        //right leg
        _drawLine(graphics, dataset,
                xHighEBPos, ypos - _ERRORBAR_LEG_LENGTH,
                xHighEBPos, ypos + _ERRORBAR_LEG_LENGTH, clip);
                ...
                try {
                    ...
                    if (fieldsplit2 > 0) {
                        ...
                        if (fieldsplit3 > 0) {
                            ...
                            addPointWithFluxErrorBars(_currentdataset, xpt, ypt,
    ...
    protected void _writeOldSyntax(PrintWriter output) {
        ...
        for (int dataset = 0; dataset < _points.size(); dataset++) {
            ...
            for (int pointnum = 0; pointnum < pts.size(); pointnum++) {
                ...
                output.print(pt.x + ", " + pt.y);

                if (pt.yErrorBar) {
                    output.print(", " + pt.yLowEB + ", "
                            + pt.yHighEB);
                }
                if (pt.xErrorBar) {
                    output.print(", " + pt.xLowEB + ", "
                            + pt.xHighEB);
                }
                output.println();
    ...
    private void _addPoint(int dataset, double x, double y, double yLowEB,
            double yHighEB, double xLowEB, double xHighEB, boolean connected, boolean xErrorBar, boolean yErrorBar) {
        // Ensure replot of offscreen buffer.
        _plotImage = null;
        ...
        if (_xlog) {
            ...
            if (xErrorBar) {
                if ((xLowEB <= 0.0) || (xHighEB <= 0.0)) {
                    System.err
                            .println("Can't plot non-positive X values "
                                    + "when the logarithmic X axis value is specified: "
                                    + x);
                    return;
                }

                xLowEB = Math.log(xLowEB) * _LOG10SCALE;
                xHighEB = Math.log(xHighEB) * _LOG10SCALE;
            }
        ...
        if (_xPersistence > 0.0) {
            ...
            while (numToDelete < pts.size()) {
                ErrorsPlotPoint old = (ErrorsPlotPoint) (pts.elementAt(numToDelete));
        ...
        ErrorsPlotPoint pt = new ErrorsPlotPoint();
        ...
        if (size == 0) {
            ...
            ErrorsPlotPoint old = (ErrorsPlotPoint) (pts.elementAt(size - 1));
        ...
        if (_wrap && (Math.abs(x - _wrapHigh)) < 0.00001) {
            ...
            _addPoint(dataset, _wrapLow, y, yLowEB, yHighEB, xLowEB, xHighEB, false, xErrorBar, yErrorBar);
        }
        /*if (_wrap && (Math.abs(x - _wrapHigh)) < 0.00001) {
            // Plot a second point at the low end of the range.
    ...
    private void _drawPlotPoint(Graphics graphics, int dataset, int index) {
        ...
        Vector pts = (Vector) _points.elementAt(dataset);
        ErrorsPlotPoint pt = (ErrorsPlotPoint) pts.elementAt(index);
        ...
        if (pt.xErrorBar) {
            _drawXErrorBar(graphics, dataset, xpos, ypos,
                    _ulx + (long) ((pt.xLowEB - _xMin) * _xscale),
                    _ulx + (long) ((pt.xHighEB - _xMin) * _xscale),
                    true);
        }

        if (pt.yErrorBar) {
            _drawYErrorBar(graphics, dataset, xpos, ypos,
                    _lry - (long) ((pt.yLowEB - _yMin) * _yscale),
                    _lry - (long) ((pt.yHighEB - _yMin) * _yscale),
                    true);
        }
    ...
    private void _erasePoint(int dataset, int index) {
        ...
        if (_showing && (graphics != null)) {
            ...
            if (index < (pts.size() - 1)) {
                ErrorsPlotPoint nextp = (ErrorsPlotPoint) pts.elementAt(index + 1);
        ...
        if (_xyInvalid) {
            ...
            for (int dataset = 0; dataset < _points.size(); dataset++) {
                ...
                for (int index = 0; index < points.size(); index++) {
                    ...
                    if (pt.xLowEB < _xBottom && pt.xErrorBar) {
                        _xBottom = pt.xLowEB;
                    }
                    ...
                    if (pt.xHighEB > _xTop && pt.xErrorBar) {
                        _xTop = pt.xHighEB;
                    }
                    ...
                    if (pt.yLowEB < _yBottom && pt.yErrorBar) {
                        _yBottom = pt.yLowEB;
                    }
                    ...
                    if (pt.yHighEB > _yTop && pt.yErrorBar) {
                        _yTop = pt.yHighEB;
                    }
