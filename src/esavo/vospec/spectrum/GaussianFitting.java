...
//import esavo.vospec.math.SpectralLine;
...
public class GaussianFitting extends ChiSquareFittingSpectrum {
    ...
    public GaussianFitting(Vector dataToFit,Unit originalUnits,boolean logX,boolean logY) {
 
        super(dataToFit,logX,logY);                
        this.setTitle("Gaussian Fitting ");
 	
	this.setUnits(originalUnits);
   }  
    
    public GaussianFitting(Spectrum spectrum, Unit originalUnits,boolean logX,boolean logY) {
    ...
    public GaussianFitting(Spectrum spectrum,boolean logX,boolean logY) {
  ...
  /*  public void calculateInitialParameters() {
	...
	parameter[SIGMA_PAR] = (xMax - xMin) /3;
...
*/
	...
	if(a < 0.) { 
        ...
        System.out.println("initialParam = "+ parameter[X0_PAR] +" "+parameter[B_PAR]);
    }  	
	...
	*/	
	...
	double function = B + A * Math.exp( (-1.) * (x - x0) * (x - x0) / 2. / sigma / sigma);
	return function;	
    }	    
	...
	double function = B + A * Math.exp( (-1.) * (x - x0) * (x - x0) / 2. / sigma / sigma);
		...
		*/	
		...
		function = (function-B) * (x - x0) / sigma / sigma;
		...
		*/	
		...
		function = (function-B) * (x - x0) * (x - x0) / sigma / sigma / sigma;
